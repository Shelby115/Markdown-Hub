...
public class AuthProviderOptionsControllerTests : IDisposable
{
    private readonly AppDbContext _db;
    private readonly AuthProviderOptionsController _sut;

    public AuthProviderOptionsControllerTests()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString())
            .Options;
        _db = new AppDbContext(options);
        _sut = new AuthProviderOptionsController(_db);
    }
